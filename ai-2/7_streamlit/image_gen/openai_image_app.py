...
#     translated_text = my_image_gen.translate_text_to_image(input_text)
...
#   image_urls = my_image_gen.genrate_image_from_text(text_for_image, image_num, image_size)
...
#     st.ssession_state.shorten_text_for_image = shorten_text_for_image
...
# input_text = st.sidebar.text_input('이미지 생성을 위한 설명을 입력하세요.', '빌딩이 보이는 호수가 있는 도시의 공원이다.')

# radio_selected_lang = st.sidebar.radio('입력한 언어', ['한국어','영어'], index=0, horizontal=True)

# image_num_options = [1, 2, 3]
# image_num = st.sidebar.radio('생성할 이미지 개수를 선택하세요.', image_num_options, index=1, horizontal=True)

# image_size_options = ['256x256', '512x512', '1024x1024']
# image_size = st.sidebar.radio('생성할 이미지의 크기를 선택해주세요.', image_size_options, index=0, horizontal=True)

# detail_description = st.sidebar.radio('상세 묘사를 추가하시나요?', ['Yes', 'No'], index=1, horizontal=True)
