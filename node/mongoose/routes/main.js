...
// mongoose 6.1.5 버전에서만 projection을 이렇게 사용!
...
app.post('/insert',async function (req, res) {
    try{
        ...
        var exisitingUser = await User.findOne({ userid: userid });
        if(exisitingUser) {
...
function query3(callback){
    // select * from users where city='Seoul' order by userrid limit 3
...
// selet userid, name from users where userid="/02/"
function query4(callback){
    User.find({userid:{$regex:"02"}},{_id:0, userid:1, name:1}).exec(function(err,user){
        console.log("\nQuery 4");
        console.log(user + "\n");
        callback(null)
    });
...
function query5(callback){
    User.find({city:"Seoul", age:{$gt:15, $lt:23}},{_id:0, userid:1, name:1, age:1}).sort({userid:-1}).select("userid name age").exec(function(err,user){
        console.log("\nQuery 5");
        console.log(user + "\n");
        callback(null)
    });
