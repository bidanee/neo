...
//     res.write(`data: [stderr] ${data.toString().replace(/\n/g,'\ndata : ')}\n\n`)
//     res.end();
//   })
// })
...
//   if (!model) return res.status(400).json({error: 'Model name and Prompt are required'});
//   const child = exec(`ollama run ${model}`,(error, stdout, stderr) => {
//     if (error) {
//       return res.status(500).json({ error: stderr || error.message });
//     }
//     res.josn({result: stdout.trim().replace(/\n/g,'<br>')});
//     });
//     childe.stdin.write(`${prompt}\n`);
//     child.stdin.end();
//   })
