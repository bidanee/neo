...
# Resources
# -------------------------------
@mcp.resource("config://apps-version")
...
def debug_session_start(error_message : str) -> list[Message] :
    ''' Initialize a debugging help seesion.'''
    return [
        {
            "role" : "user",
...
async def process_large_file(file_uri : str, ctx : Context) -> str :
    """Processes a large file and reporting progress and reading resources. Supports both MCP resource URIs and file:/// URIs."""
    ...
    if file_uri.startswith("file://") :
        ...
        if not os.path.exists(path) :
            await ctx.error(f"File not foun : {path}")
