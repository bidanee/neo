...
base_url = 'http://192.168.1.19:5000/users'
...
async def users_params(id:Union[str,None] = None, name:Union[str,None] = None):
...
async def users_params(id:Optional[str] = None, name:Optional[str] = None):
    if(id is None) and (name is None):
        return "id, name을 입력하세요."
    else:
        if id is None:
...
async def users_param(id=str):
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail="Invalid JSON respnose from server")
